...
namespace GameFramework.Resource
{
    ...
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        {
            ResourceGroup resourceGroup = new ResourceGroup("temp", m_ResourceInfos);
            foreach (var name in resNameList)
            {
                AssetInfo assetInfo;
                m_AssetInfos.TryGetValue(name, out assetInfo);
                {
                    ResourceName rn = assetInfo.ResourceName;
                    if (m_ResourceInfos.ContainsKey(rn))
                    {
                        var resInfo = m_ResourceInfos[rn];
                        resourceGroup.AddResource(rn, resInfo.Length, resInfo.CompressedLength);
                    }
                }
            }
